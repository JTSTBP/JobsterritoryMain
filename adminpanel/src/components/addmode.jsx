...
export default function AddPage() {
  ...
  // Initialize defaults for select fields
  useEffect(() => {
    const defaults = {};
    if (fields) {
      fields.forEach((f) => {
  ...
  const handleChange = (e, key) => {
    ...
    if (files) {
      ...
      if (key === "heading") {
        ...
        const isSlugAutoGenerated = !formData.slug || formData.slug === oldSlugified;
    ...
    try {
      ...
      for (const key in formData) {
        ...
        if (value instanceof File) {
          ...
          // ✅ Normal string/number - Skip empty strings for non-required fields to avoid CastErrors (e.g. Date)
